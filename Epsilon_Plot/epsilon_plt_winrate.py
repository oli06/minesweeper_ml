...
with open('epsilon90.txt') as f:
    lines = f.readlines()
    x1 = [i * 100 for i in range(len(lines[:10]))]
    y1 = [float(line.split(',')[0]) for line in lines[:10]]

with open('epsilon35.txt') as g:
    lines = g.readlines()
    x2 = [i * 100 for i in range(len(lines[:10]))]
    y2 = [float(line.split(',')[0]) for line in lines[:10]]

with open('epsilon1.txt') as h:
    lines = h.readlines()
    x3 = [i * 100 for i in range(len(lines[:10]))]
    y3 = [float(line.split(',')[0]) for line in lines[:10]]        
